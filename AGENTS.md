...
- 모든 파일은 반드시 **UTF-8**로 저장한다.
- ASCII, ANSI 등 UTF-8이 아닌 인코딩 사용 금지.

- 한글이 포함된 텍스트는 절대 수정하거나 깨뜨리지 않는다.
- 한글을 유니코드 이스케이프(\uXXXX)로 변환하지 않는다.

- 파일 수정 시 전체 재작성 금지, 필요한 부분만 수정한다.

- 저장 전/후 한글이 정상인지 반드시 확인한다.
- 인코딩이 불확실하면 파일을 저장하지 않는다.
